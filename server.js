...
// построчная эвристика (на случай JSON-логов «в одну строку»)
...
// поиск по всему хвосту (для многострочного JSON-блока)
function tailContainsPair(tail, chatId, authorId) {
  const chatNeedle = `"chat_id": "${chatId}"`;
  const authNeedle1 = `"author_id": ${authorId}`;
  const authNeedle2 = `"author_id": "${authorId}"`;
...
app.post('/tasks/doneSafe', async (req, res) => {
  ...
  try {
    ...
    authorId = task?.author_id ?? task?.account_id ?? null;
  } catch (e) {
  ...
  if (!confirmed) {
    // На отладке можно вернуть детали:
    // return res.status(428).send({ ok:false, confirmed:false, chat_id: chatId, author_id: authorId });
